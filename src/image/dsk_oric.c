...
static bool_t dsk_oric_open(struct image *im)
{
...
static void dsk_oric_seek_track(
    ...
    struct image *im, uint16_t track, uint32_t *start_pos)
{
    ...
    if (track != im->cur_track)
        dsk_oric_seek_track(im, track, cyl, side);
...
static bool_t dsk_oric_read_track(struct image *im)
{
    struct tib *tib = tib_p(im);
    struct image_buf *rd = &im->bufs.read_data;
    struct image_buf *bc = &im->bufs.read_bc;
...
static bool_t dsk_oric_write_track(struct image *im)
{
...
const struct image_handler dsk_oric_image_handler = {
    .open = dsk_oric_open,
    .setup_track = dsk_oric_setup_track,
    .read_track = dsk_oric_read_track,
    .rdata_flux = bc_rdata_flux,
    .write_track = dsk_oric_write_track,
};
